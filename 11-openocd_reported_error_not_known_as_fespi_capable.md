...
```
因为在openocd 2023.10中，将`flash bank $_FLASHNAME`从`fespi`修改为了`nuspi`，需要工程中的openocd配置文件中的`fespi`修改为了`nuspi`，以蜂鸟工程为例，将`hbird_sdk/SoC/hbirdv2/Board/mcu200t/openocd_hbirdv2.cfg`修改为如下配置，工程即可正常使用。
